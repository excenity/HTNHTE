...
CF_analysis = function(htn_med_class_i, df, outcome)
{
  if (outcome == 'at_control_14090')
  {
    ite = read.csv(file.path(path, paste0('results/step1_ITE_estimation/ITE_at_control_14090_', htn_med_class_i, '.csv')), col.names = c('pid', 'ite'))
  } else if (outcome == 'at_control_13080')
  {
    ite = read.csv(file.path(path, paste0('results/step1_ITE_estimation/ITE_at_control_13080_', htn_med_class_i, '.csv')), col.names = c('pid', 'ite'))
  } else if (outcome == 'sbp_change')
  {
    ite = read.csv(file.path(path, paste0('results/step1_ITE_estimation/ITE_sbp_change_', htn_med_class_i, '.csv')), col.names = c('pid', 'ite'))
  }
